...
#[inline]
fn sink_state_from_raw(state: capi::pa_sink_state_t) -> def::SinkState {
    state.into()
}

#[inline]
fn source_state_from_raw(state: capi::pa_source_state_t) -> def::SourceState {
    state.into()
}
    ...
    fn new_from_raw(p: *const SinkInfoInternal) -> Self {
        ...
        unsafe {
            SinkInfo {
                ...
                state: sink_state_from_raw(src.state),
    ...
    fn new_from_raw(p: *const SourceInfoInternal) -> Self {
        ...
        unsafe {
            SourceInfo {
                ...
                configured_latency: MicroSeconds(src.configured_latency),
                base_volume: Volume(src.base_volume),
                state: source_state_from_raw(src.state),
                n_volume_steps: src.n_volume_steps,
                card: match src.card {
                    def::INVALID_INDEX => None,
                    i => Some(i),
...
mod state_conversion_tests {
    use super::{sink_state_from_raw, source_state_from_raw};
    use crate::def::{SinkState, SourceState};
    use crate::capi;

    #[test]
    fn sink_info_conversion_handles_private_and_unknown_states() {
        assert_eq!(SinkState::Unlinked, sink_state_from_raw(capi::PA_SINK_UNLINKED));
        assert_eq!(SinkState::Init, sink_state_from_raw(capi::PA_SINK_INIT));
        assert_eq!(SinkState::Invalid, sink_state_from_raw(capi::PA_SINK_INVALID_STATE));
        assert_eq!(SinkState::Running, sink_state_from_raw(capi::PA_SINK_RUNNING));
        assert_eq!(SinkState::Idle, sink_state_from_raw(capi::PA_SINK_IDLE));
        assert_eq!(SinkState::Suspended, sink_state_from_raw(capi::PA_SINK_SUSPENDED));
        assert_eq!(SinkState::Unknown(-99), sink_state_from_raw(capi::pa_sink_state_t::from_raw(-99)));
    }

    #[test]
    fn source_info_conversion_handles_private_and_unknown_states() {
        assert_eq!(SourceState::Unlinked, source_state_from_raw(capi::PA_SOURCE_UNLINKED));
        assert_eq!(SourceState::Init, source_state_from_raw(capi::PA_SOURCE_INIT));
        assert_eq!(SourceState::Invalid, source_state_from_raw(capi::PA_SOURCE_INVALID_STATE));
        assert_eq!(SourceState::Running, source_state_from_raw(capi::PA_SOURCE_RUNNING));
        assert_eq!(SourceState::Idle, source_state_from_raw(capi::PA_SOURCE_IDLE));
        assert_eq!(SourceState::Suspended, source_state_from_raw(capi::PA_SOURCE_SUSPENDED));
        assert_eq!(SourceState::Unknown(99), source_state_from_raw(capi::pa_source_state_t::from_raw(99)));
    }
}
